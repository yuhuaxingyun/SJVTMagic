...
- (void)updateSelectedItem:(BOOL)transformScale {
    ...
    _selectedItem.imageView.layer.transform = CATransform3DMakeScale(_itemScale, _itemScale, _itemScale);
    if (![originalItem isEqual:_selectedItem]) {
        originalItem.titleLabel.layer.transform = CATransform3DIdentity;
        originalItem.imageView.layer.transform = CATransform3DIdentity;
    }
}
...
- (void)resetFramesForDefault {
    ...
    for (int index = 0; index < _menuTitles.count; index++) {
        ...
        if (!itemWidth) {
               ...
               if ([[_menuTypes objectAtIndex:index] isEqualToString:kIsImageType]) {
                   itemWidth = height;
               }else{
...
- (UIButton *)loadItemAtIndex:(NSInteger)index {
    ...
    if (menuItem) {
        ...
        menuItem.imageView.layer.transform = CATransform3DIdentity;
        [menuItem setImageEdgeInsets:UIEdgeInsetsMake(5, 0, 5, 0)];
