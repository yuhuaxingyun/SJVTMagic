...
typedef struct {
    ...
    unsigned int dataSourceMenuTypes : 1;
...
@property (nonatomic, strong) NSArray *menuTypes; // 顶部分类类型数组（可选类型：如果是有图片必须设置，只是title不需要设置）
...
- (void)reloadDataWithDisIndex:(NSInteger)disIndex {
    ...
    if (_magicFlags.dataSourceMenuTitles) {
        ...
        if ([_dataSource respondsToSelector:@selector(menuTypesForMagicView:)]) {
            _menuTypes = [_dataSource menuTypesForMagicView:self];
            _menuBar.menuTypes = _menuTypes;
        }
...
- (void)graduallyChangeColor {
    ...
    currentItem.imageView.layer.transform = CATransform3DMakeScale(currentScale, currentScale, currentScale);
    nextItem.imageView.layer.transform = CATransform3DMakeScale(nextScale, nextScale, nextScale);
}
...
- (UIButton *)menuBar:(VTMenuBar *)menuBar menuItemAtIndex:(NSUInteger)itemIndex {
    ...
    if ([[_menuTypes objectAtIndex:itemIndex] isEqualToString:kIsImageType]&&_menuTypes != nil) {
        [menuItem setTitle:nil forState:UIControlStateNormal];
        [menuItem setImage:[UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",_menuTitles[itemIndex]]]]] forState:UIControlStateNormal];
    }else{
...
- (void)setDataSource:(id<VTMagicViewDataSource>)dataSource {
    ...
    _magicFlags.dataSourceMenuTypes = [dataSource respondsToSelector:@selector(menuTypesForMagicView:)];
